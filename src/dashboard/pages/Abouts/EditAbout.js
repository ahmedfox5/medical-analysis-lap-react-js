...
import { getAbout ,updateAbout } from './../../../api/abouts'; 
...
function EditAbout(props){
    ...
    useEffect(()=>{
        props.setPage();
        props.startLoading();
        getAbout(props.match.params.id).then(response => {
            if(response.data.success){
                setTitle(response.data.about.title);
                setDescription(response.data.about.description);
                setImg('about/' + response.data.about.img);
            }else{
                props.serverError();
            }
        }).then(()=>{
    ...
    return(
        <div className={'dashboard p-3'}>
            <Auth />
            <h4>
                <Link to='/dashboard/abouts'>Abouts</Link> / Edit About
            </h4>
            <br/><br/>
            <Formik 
